...
<<<<<<< HEAD
        ...
        {
            var user = await _userManager.FindUserByClaimsPrincipleWithAddress
        (User);
...
=======
    ...
    public async Task<ActionResult<UserDto>> Register(RegisterDto registerDto)
    {
        if( CheckEmailExistsAsync(registerDto.Email).Result.Value)
        {
            return new BadRequestObjectResult(new ApiValidationErrorResponse{ Errors= new []{"Email adddress is in use"}});
        }
